...
### Added
- Bulk Prefix for Notebooks and Dashboards: select multiple owned documents, enter a prefix, and prepend it to all their names in one action. Shows an old → new name preview before applying; any document whose resulting name would exceed 128 characters is skipped individually (reported as a per-item failure) rather than aborting the whole batch.
...
- Name length validation (128 characters, the Document API's limit) for both rename paths, in `ui/app/utils/validation.ts` (`validateDocumentName`, unit tested) on the client and again in `notebooksUpdate.function.ts`/`dashboardsUpdate.function.ts` server-side.
