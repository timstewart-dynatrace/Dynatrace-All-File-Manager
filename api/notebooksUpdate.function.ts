...
export default async function (payload: UpdatePayload) {
  try {
    ...
    if (!version) {
      throw new Error(`Unable to retrieve version for notebook ${payload.id} — cannot safely update`);
    }
    ...
    if (payload.name !== undefined && payload.isPrivate !== undefined) {
      ...
      message = `Notebook renamed and updated to ${visibility} successfully`;
    ...
    console.log(`Notebook ${payload.id}: ${message}`);
