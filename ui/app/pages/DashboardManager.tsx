              ...
              {loading ? (
                ...
                >
                  {filteredAndSortedDashboards
                    .filter((dashboard) => !showShareUrls || shares.has(dashboard.id))
                    .map((dashboard) => (
                      <div key={dashboard.id} style={{ marginBottom: "8px" }}>
                        {showShareUrls && shares.has(dashboard.id) ? (
                          <a
                            href={generateShareUrl(shares.get(dashboard.id)!.id)}
                            target="_blank"
                  ...
                  {showShareUrls && filteredAndSortedDashboards.filter((d) => shares.has(d.id)).length === 0 && (
                    <Paragraph style={{ color: Colors.Text.Neutral.Subdued, fontStyle: "italic" }}>
                      No dashboards with share links. Generate share links first.
                    </Paragraph>
                  )}
                </div>
              ) : (
